...
2. To List all the databases
		`\l`

3. To Switch to another database
		`\c <another_database_name>`

4. To List all the tables
		`\dt`



###  Compose File (Running DB server)
...
#  Postgres SQL config
  datasource:
    ...
    url: jdbc:postgres://localhost:5432/student
    username: myuser
    password: secret
...
https://www.youtube.com/watch?v=qteUUBPy2iE&t=357s
...
-- PROBLEM 1
SELECT b.category, count(*) as number_of_bikes from bike b group by category having count(*)>2;
...
-- PROBLEM 2
SELECT cu.name, count(me.id) as membership_count from customer cu LEFT OUTER JOIN membership me on cu.id=me.customer_id group by cu.name order by count(me.id) desc;
...
b.id,
b.category,
b.price_per_hour as old_price_per_hour,
...
b.price_per_day as old_price_per_day,
